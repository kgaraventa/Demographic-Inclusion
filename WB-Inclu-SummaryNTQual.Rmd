...
wb_inclusion_comments_positive <- wb_inclusion_comments %>% 
  filter(str_detect(id_code, ".1$"))

if (sum(str_detect(wb_inclusion_comments_positive$id_comment, "^XX")) > 0) {
  wb_inclusion_comments_positive %>% 
    select(id_comment) %>% 
    drop_na()  %>% 
    distinct() %>% 
    gt() %>% 
...
```{r improve responses}
wb_inclusion_comments_improve <- wb_inclusion_comments %>% 
  filter(str_detect(id_code, ".2$"))

if (sum(str_detect(wb_inclusion_comments_improve$id_comment, "^XX")) > 0) {
  wb_inclusion_comments_improve %>% 
    select(id_comment) %>% 
    drop_na()  %>% 
    distinct() %>% 
    gt() %>% 
