...
ee_inclusion <- read_excel(path = "G:\\Shared drives\\CWP Training Evaluation Protected\\CWP Training\\EE\\Data\\EE DB 202307.xlsx",
                                   sheet = "overall") %>% 
  clean_names() %>% 
...
```{r}
ee_inclusion_comments <- ee_inclusion %>% 
  filter(quarter == params$quarter) %>% 
...
number_positive <- if (sum(str_detect(ee_inclusion_comments$id_code, ".1$")) > 0) {
  ee_inclusion_comments %>%
...
number_positive_codes <- if (sum(str_detect(ee_inclusion_comments$id_code, ".1$")) > 0) {
  ee_inclusion_comments %>%
    filter(str_detect(id_code, ".1$")) %>%
    count(quarter) %>%
    pull(n)
...
```{r positive prep}
positive_theme_count <- ee_inclusion_comments %>% 
  filter(str_detect(id_code, ".1$")) %>% 
  count(id_code) %>% 
  arrange(desc(n)) %>% 
...
positive_theme_count %>% 
  ...
  scale_fill_manual(values = c(
                     ...
                     "Highlighted Comment - positive" = psu_electricgreen)) +
...
ee_inclusion_comments_positive <- ee_inclusion_comments %>% 
  filter(str_detect(id_code, ".1$"))

if (sum(str_detect(ee_inclusion_comments_positive$id_comment, "^XX")) > 0) {
  ee_inclusion_comments_positive %>% 
    select(id_comment) %>% 
    drop_na()  %>% 
    distinct() %>% 
    gt() %>% 
...
number_improve <- if (sum(str_detect(ee_inclusion_comments$id_code, ".2$")) > 0) {
  ee_inclusion_comments %>%
...
number_improve_codes <- if (sum(str_detect(ee_inclusion_comments$id_code, ".2$")) > 0) {
  ee_inclusion_comments %>%
    filter(str_detect(id_code, ".2$")) %>%
    count(quarter) %>%
    pull(n)
...
```{r improve prep}
improve_theme_count <- ee_inclusion_comments %>% 
  filter(str_detect(id_code, ".2$")) %>% 
  count(id_code) %>% 
  arrange(desc(n)) %>% 
  left_join(qual_codes,
...
improve_theme_count %>% 
  ...
  scale_fill_manual(values = c(
                     ...
                     "Highlighted Comment - to improve" = psu_electricgreen)) +
  geom_col(show.legend = FALSE,
           position = "dodge") +
...
```{r improve responses}
ee_inclusion_comments_improve <- ee_inclusion_comments %>% 
  filter(str_detect(id_code, ".2$"))

if (sum(str_detect(ee_inclusion_comments_improve$id_comment, "^XX")) > 0) {
  ee_inclusion_comments_improve %>% 
    select(id_comment) %>% 
    drop_na()  %>% 
    distinct() %>% 
    gt() %>% 
