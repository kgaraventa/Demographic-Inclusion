...
fc_inclusion <- read_excel(path = "G:\\Shared drives\\CWP Training Evaluation Protected\\CWP Training\\FC\\Data\\FC DB 202307.xlsx",
                                   sheet = "overall") %>% 
  clean_names() %>% 
...
```{r}
fc_inclusion_comments <- fc_inclusion %>% 
  filter(quarter == params$quarter) %>% 
...
number_positive <- if (sum(str_detect(fc_inclusion_comments$id_comment_code, ".1$")) > 0) {
  fc_inclusion_comments %>%
...
number_positive_codes <- if (sum(str_detect(fc_inclusion_comments$id_comment_code, ".1$")) > 0) {
  fc_inclusion_comments %>%
    filter(str_detect(id_comment_code, ".1$")) %>%
    count(quarter) %>%
    pull(n)
...
```{r positive prep}
positive_theme_count <- fc_inclusion_comments %>% 
  filter(str_detect(id_comment_code, ".1$")) %>% 
...
fc_inclusion_comments_positive <- fc_inclusion_comments %>% 
  filter(str_detect(id_comment_code, ".1$"))

if (sum(str_detect(fc_inclusion_comments_positive$id_comment, "^XX")) > 0) {
  fc_inclusion_comments_positive %>% 
    select(id_comment) %>% 
    drop_na()  %>% 
    distinct() %>% 
    gt() %>% 
...
number_improve <- if (sum(str_detect(fc_inclusion_comments$id_comment_code, ".2$")) > 0) {
  fc_inclusion_comments %>%
...
number_improve_codes <- if (sum(str_detect(fc_inclusion_comments$id_comment_code, ".2$")) > 0) {
  fc_inclusion_comments %>%
    filter(str_detect(id_comment_code, ".2$")) %>%
    count(quarter) %>%
    pull(n)
...
```{r improve prep}
improve_theme_count <- fc_inclusion_comments %>% 
  filter(str_detect(id_comment_code, ".2$")) %>% 
  count(id_comment_code) %>% 
  arrange(desc(n)) %>% 
  left_join(qual_codes,
...
```{r improve responses}
fc_inclusion_comments_improve <- fc_inclusion_comments %>% 
  filter(str_detect(id_comment_code, ".2$"))

if (sum(str_detect(fc_inclusion_comments_improve$id_comment, "^XX")) > 0) {
  fc_inclusion_comments_improve %>% 
    select(id_comment) %>% 
    drop_na()  %>% 
    distinct() %>% 
    gt() %>% 
