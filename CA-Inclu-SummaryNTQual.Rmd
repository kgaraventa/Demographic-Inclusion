...
```{r}
ca_inclusion_comments <- ca_inclusion %>% 
  filter(quarter == params$quarter) %>% 
...
number_positive <- if (sum(str_detect(ca_inclusion_comments$id_code, ".1$")) > 0) {
  ca_inclusion_comments %>%
...
number_positive_codes <- if (sum(str_detect(ca_inclusion_comments$id_code, ".1$")) > 0) {
  ca_inclusion_comments %>%
    filter(str_detect(id_code, ".1$")) %>%
    count(quarter) %>%
    pull(n)
...
```{r positive prep}
positive_theme_count <- ca_inclusion_comments %>% 
  filter(str_detect(id_code, ".1$")) %>% 
  count(id_code) %>% 
  arrange(desc(n)) %>% 
...
ca_inclusion_comments_positive <- ca_inclusion_comments %>% 
  filter(str_detect(id_code, ".1$"))

if (sum(str_detect(ca_inclusion_comments_positive$id_comment, "^XX")) > 0) {
  ca_inclusion_comments_positive %>% 
    select(id_comment) %>% 
    drop_na()  %>% 
    distinct() %>% 
    gt() %>% 
...
number_improve <- if (sum(str_detect(ca_inclusion_comments$id_code, ".2$")) > 0) {
  ca_inclusion_comments %>%
...
number_improve_codes <- if (sum(str_detect(ca_inclusion_comments$id_code, ".2$")) > 0) {
  ca_inclusion_comments %>%
    filter(str_detect(id_code, ".2$")) %>%
    count(quarter) %>%
    pull(n)
...
```{r improve prep}
improve_theme_count <- ca_inclusion_comments %>% 
  filter(str_detect(id_code, ".2$")) %>% 
  count(id_code) %>% 
  arrange(desc(n)) %>% 
  left_join(qual_codes,
...
```{r improve responses}
ca_inclusion_comments_improve <- ca_inclusion_comments %>% 
  filter(str_detect(id_code, ".2$"))

if (sum(str_detect(ca_inclusion_comments_improve$id_comment, "^XX")) > 0) {
  ca_inclusion_comments_improve %>% 
    select(id_comment) %>% 
    drop_na()  %>% 
    distinct() %>% 
    gt() %>% 
