...
ppsc_inclusion <- read_excel(path = "G:\\Shared drives\\CWP Training Evaluation Protected\\CWP Training\\PPSC\\Data\\PPSC DB 202307.xlsx",
                                   sheet = "overall") %>% 
  clean_names() %>% 
  select(cohort, id_trainer, id_other, id_comment, id_comment_code) %>% 
...
```{r}
ppsc_inclusion_scales <- ppsc_inclusion %>%
  select(-c(cohort, id_comment, id_comment_code)) %>% 
...
avg_trainers <- ppsc_inclusion_scales %>%
  filter(quarter == params$quarter,
         scale == "id_trainer") %>%
  drop_na() %>% 
...
```{r peer rating prep}
avg_peers <- ppsc_inclusion_scales %>%
  filter(quarter == params$quarter,
         scale == "id_other") %>%
  drop_na() %>% 
...
```{r}
ppsc_inclusion_comments <- ppsc_inclusion %>% 
  filter(quarter == params$quarter) %>% 
  drop_na(id_comment, id_comment_code) %>% 
  separate_longer_delim(id_comment_code,
                        delim = ", ") %>% 
  mutate(id_comment_code = as.numeric(id_comment_code))
```

```{r positive comment count}
number_positive <- if (sum(str_detect(ppsc_inclusion_comments$id_comment_code, ".1$")) > 0) {
  ppsc_inclusion_comments %>%
    filter(str_detect(id_comment_code, ".1$")) %>%
...
number_positive_codes <- if (sum(str_detect(ppsc_inclusion_comments$id_comment_code, ".1$")) > 0) {
  ppsc_inclusion_comments %>%
    filter(str_detect(id_comment_code, ".1$")) %>%
    count(quarter) %>%
    pull(n)
...
```{r positive prep}
positive_theme_count <- ppsc_inclusion_comments %>% 
  filter(str_detect(id_comment_code, ".1$")) %>% 
  count(id_comment_code) %>% 
  arrange(desc(n)) %>% 
  left_join(qual_codes,
            join_by(id_comment_code == code))
...
ppsc_inclusion_comments_positive <- ppsc_inclusion_comments %>% 
  filter(str_detect(id_comment_code, ".1$"))

if (sum(str_detect(ppsc_inclusion_comments_positive$id_comment, "^XX")) > 0) {
  ppsc_inclusion_comments_positive %>% 
    select(id_comment) %>% 
    drop_na()  %>% 
    distinct() %>% 
    gt() %>% 
...
number_improve <- if (sum(str_detect(ppsc_inclusion_comments$id_comment_code, ".2$")) > 0) {
  ppsc_inclusion_comments %>%
    filter(str_detect(id_comment_code, ".2$")) %>%
    distinct(id_comment, .keep_all = TRUE) %>% 
    count(quarter) %>%
    pull(n)
} else {
...
number_improve_codes <- if (sum(str_detect(ppsc_inclusion_comments$id_comment_code, ".2$")) > 0) {
  ppsc_inclusion_comments %>%
    filter(str_detect(id_comment_code, ".2$")) %>%
    count(quarter) %>%
    pull(n)
...
```{r improve prep}
improve_theme_count <- ppsc_inclusion_comments %>% 
  filter(str_detect(id_comment_code, ".2$")) %>% 
  count(id_comment_code) %>% 
  arrange(desc(n)) %>% 
  left_join(qual_codes,
...
improve_theme_count %>% 
  ...
  theme(panel.grid.major.x = element_blank(),
        ...
        panel.grid.major.y = element_blank(),
        axis.title = element_blank(),
        plot.title = element_markdown(face = "bold", hjust = .1))
```

```{r improve responses}
ppsc_inclusion_comments_improve <- ppsc_inclusion_comments %>% 
  filter(str_detect(id_comment_code, ".2$"))

if (sum(str_detect(ppsc_inclusion_comments_improve$id_comment, "^XX")) > 0) {
  ppsc_inclusion_comments_improve %>% 
    select(id_comment) %>% 
    drop_na()  %>% 
    distinct() %>% 
    gt() %>% 
