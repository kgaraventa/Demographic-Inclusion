...
tips_inclusion <- read_excel(path = "G:\\Shared drives\\CWP Training Evaluation Protected\\CWP Training\\TIPS\\Data\\TIPS DB 202307.xlsx",
                                   sheet = "session") %>% 
  clean_names() %>% 
...
```{r}
tips_inclusion_scales <- tips_inclusion %>%
...
avg_trainers <- tips_inclusion_scales %>%
  filter(quarter == params$quarter,
         scale == "id_trainer") %>%
  drop_na() %>% 
...
```{r peer rating prep}
avg_peers <- tips_inclusion_scales %>%
  filter(quarter == params$quarter,
         scale == "id_other") %>%
  drop_na() %>% 
...
```{r}
tips_inclusion_comments <- tips_inclusion %>% 
  filter(quarter == params$quarter) %>% 
...
number_positive <- if (sum(str_detect(tips_inclusion_comments$id_code, ".1$")) > 0) {
  tips_inclusion_comments %>%
...
number_positive_codes <- if (sum(str_detect(tips_inclusion_comments$id_code, ".1$")) > 0) {
  tips_inclusion_comments %>%
    filter(str_detect(id_code, ".1$")) %>%
    count(quarter) %>%
    pull(n)
...
```{r positive prep}
positive_theme_count <- tips_inclusion_comments %>% 
  filter(str_detect(id_code, ".1$")) %>% 
  count(id_code) %>% 
  arrange(desc(n)) %>% 
...
tips_inclusion_comments_positive <- tips_inclusion_comments %>% 
  filter(str_detect(id_code, ".1$"))

if (sum(str_detect(tips_inclusion_comments_positive$id_comment, "^XX")) > 0) {
  tips_inclusion_comments_positive %>% 
    select(id_comment) %>% 
    drop_na()  %>% 
    distinct() %>% 
    gt() %>% 
...
number_improve <- if (sum(str_detect(tips_inclusion_comments$id_code, ".2$")) > 0) {
  tips_inclusion_comments %>%
...
number_improve_codes <- if (sum(str_detect(tips_inclusion_comments$id_code, ".2$")) > 0) {
  tips_inclusion_comments %>%
    filter(str_detect(id_code, ".2$")) %>%
    count(quarter) %>%
    pull(n)
...
```{r improve prep}
improve_theme_count <- tips_inclusion_comments %>% 
  filter(str_detect(id_code, ".2$")) %>% 
  count(id_code) %>% 
  arrange(desc(n)) %>% 
  left_join(qual_codes,
...
```{r improve responses}
tips_inclusion_comments_improve <- tips_inclusion_comments %>% 
  filter(str_detect(id_code, ".2$"))

if (sum(str_detect(tips_inclusion_comments_improve$id_comment, "^XX")) > 0) {
  tips_inclusion_comments_improve %>% 
    select(id_comment) %>% 
    drop_na()  %>% 
    distinct() %>% 
    gt() %>% 
