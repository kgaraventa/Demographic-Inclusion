...
subtitle: "Training: Social Services Assistant Training"
...
ssa_inclusion <- read_excel(path = "G:\\Shared drives\\CWP Training Evaluation Protected\\CWP Training\\SSA\\Data\\SSA DB 202307.xlsx",
                                   sheet = "overall") %>% 
  clean_names() %>% 
...
number_respondents_inclusion <- ssa_inclusion %>% 
...
```{r}
ssa_inclusion_scales <- ssa_inclusion %>%
...
avg_trainers <- ssa_inclusion_scales %>%
  filter(quarter == params$quarter,
         scale == "id_trainer") %>%
  drop_na() %>% 
...
```{r peer rating prep}
avg_peers <- ssa_inclusion_scales %>%
  filter(quarter == params$quarter,
         scale == "id_other") %>%
  drop_na() %>% 
...
```{r}
ssa_inclusion_comments <- ssa_inclusion %>% 
  filter(quarter == params$quarter) %>% 
...
number_positive <- if (sum(str_detect(ssa_inclusion_comments$id_code, ".1$")) > 0) {
  ssa_inclusion_comments %>%
...
number_positive_codes <- if (sum(str_detect(ssa_inclusion_comments$id_code, ".1$")) > 0) {
  ssa_inclusion_comments %>%
    filter(str_detect(id_code, ".1$")) %>%
    count(quarter) %>%
    pull(n)
...
```{r positive prep}
positive_theme_count <- ssa_inclusion_comments %>% 
  filter(str_detect(id_code, ".1$")) %>% 
  count(id_code) %>% 
  arrange(desc(n)) %>% 
...
ssa_inclusion_comments_positive <- ssa_inclusion_comments %>% 
  filter(str_detect(id_code, ".1$"))

if (sum(str_detect(ssa_inclusion_comments_positive$id_comment, "^XX")) > 0) {
  ssa_inclusion_comments_positive %>% 
    select(id_comment) %>% 
    drop_na()  %>% 
    distinct() %>% 
    gt() %>% 
...
number_improve <- if (sum(str_detect(ssa_inclusion_comments$id_code, ".2$")) > 0) {
  ssa_inclusion_comments %>%
...
number_improve_codes <- if (sum(str_detect(ssa_inclusion_comments$id_code, ".2$")) > 0) {
  ssa_inclusion_comments %>%
    filter(str_detect(id_code, ".2$")) %>%
    count(quarter) %>%
    pull(n)
...
```{r improve prep}
improve_theme_count <- ssa_inclusion_comments %>% 
  filter(str_detect(id_code, ".2$")) %>% 
  count(id_code) %>% 
  arrange(desc(n)) %>% 
  left_join(qual_codes,
...
```{r improve responses}
ssa_inclusion_comments_improve <- ssa_inclusion_comments %>% 
  filter(str_detect(id_code, ".2$"))

if (sum(str_detect(ssa_inclusion_comments_improve$id_comment, "^XX")) > 0) {
  ssa_inclusion_comments_improve %>% 
    select(id_comment) %>% 
    drop_na()  %>% 
    distinct() %>% 
    gt() %>% 
